...
const debug = require('debug')('rtsp:debug');
const error = require('debug')('rtsp:error');
		...
		this.socket
			...
			.on("data",(chunk)=>{
				...
				try {
					...
					//Log
					debug("REQUEST:\n %s", this.pending.substring(0,request.messageSize));
					...
					//Check
					if (!cseq)
						return error("cseq %d not found", cseq);
